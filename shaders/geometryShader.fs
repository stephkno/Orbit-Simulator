...
layout (location = 3) out vec3 gSunLightMap;
//layout (location = 4) out vec4 gMousePickBuffer;
...
float ShadowCalculation(vec4 fragPosLightSpace)
{
    ...
    // PCF
    float shadow = 0.0;
    vec2 texelSize = 1.0 / materialize(shadow_texture, 0);
    for(int x = -1; x <= 1; ++x)
    {
        for(int y = -1; y <= 1; ++y)
        {
...
void main()
{
  ...
  vec3 n0 = texture(material[0].normalTexture, TexCoords).rgb;
  vec3 d0 = texture(material[0].diffuseTexture, TexCoords).rgb;
  vec3 e0 = texture(material[0].emissiveTexture, TexCoords).rgb;
  vec3 s0 = texture(material[0].specularTexture, TexCoords).rgb;
  ...
  if(material[0].normalScale > 0){
    gSunLightMap.rgb = SunLight(n0*Normal);
    gNormal = normalize(Normal) + (n0 * material[0].normalScale);
  }else{
    gSunLightMap.rgb = SunLight(vec3(1.0));
